    ...
    if os.path.isdir(fuzzer_dir):
        for item in os.listdir(fuzzer_dir):
            ...
            if os.path.isfile(item_path) and os.access(item_path, os.X_OK):
                # Skip coverage builds and other non-fuzzer executables (match fuzzer/discovery.go: skip .json etc.)
...
def run_fuzzer_with_coverage(log_file, fuzzer_path, project_dir, focus, sanitizer, project_name, seed_corpus_dir):
    ...
    try:
        ...
        docker_cmd = ["timeout", "-k", "10", "60"] + docker_cmd  # 60 s hard limit
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            try:
                ...
                log_message(log_file, f"Error parsing response: {e}")
...
def doAdvancedPoV_full(log_file,fuzzer_src_path, fuzzer_code, fuzzer_path, fuzzer_name, sanitizer, project_dir, project_name, focus, language='c', check_patch_success=False) -> bool:
    ...
    if POV_PHASE == 0:
        ...
        if len(all_reachable_funcs) > 10:
            ...
            if language.startswith('j'):
                model_name = OPENAI_MODEL_O3
            top_k = len(all_reachable_funcs) // 5
            if top_k > 10:
                top_k = 10            
            vulnerable_functions = find_most_likely_vulnerable_functions(log_file,all_reachable_funcs,language,model_name,top_k)
            reachable_funcs = extract_vulnerable_functions(reachable_funcs,vulnerable_functions)
        ...
        # log_message(log_file, f"reachable_funcs: {reachable_funcs}")
        ...
        if not pov_success and len(call_paths)>1:
            ...
            initial_msg = create_commit_combine_all_call_paths_based_prompt(project_name,fuzzer_code, commit_diff, project_src_dir,call_paths,sanitizer,language)
            line_count = initial_msg.count('\n') + 1
            char_length = len(initial_msg)
            print(f"initial_msg: {line_count} lines, {char_length} characters")
            print(f"{initial_msg}")
...
def create_commit_vul_category_based_prompt_for_c(project_name,fuzzer_code, commit_diff, sanitizer, category):
    category_desc = {
...
def create_commit_vul_category_based_prompt_for_java(project_name,fuzzer_code, commit_diff, sanitizer, category):
    category_desc = {
